...
import { useOutflows } from '../../api/useFlows'
...
function SchemeSection({ group, onSelect }: { group: SchemeGroup; onSelect: (c: FlowCounterparty) => void }) {
  ...
  const sorted = [...group.records].sort((a, b) => {
    ...
    if (sortField === 'name') return mul * a.payee_name.localeCompare(b.payee_name)
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-stone-100">
          {sorted.map((r) => (
            ...
            >
              <td className="px-4 py-1.5 text-stone-700">{r.payee_name}</td>
              <td className="px-4 py-1.5 text-stone-400 text-xs capitalize">
                {r.payee_entity_type.replace(/_/g, ' ')}
              </td>
...
export default function OutflowsTab({ entity }: Props) {
  const [year, setYear]             = useState(2024)
  const [counterparty, setCounterparty] = useState<FlowCounterparty | null>(null)
  const { records, loading, error } = useOutflows(entity.entity, year)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="font-serif text-xl font-bold text-stone-800 mb-1">Outflows</h2>
        <p className="text-sm text-stone-500">Payments and grants made by {entity.display}.</p>
      </div>
      ...
      {!loading && records.length === 0 && !error && (
        <div className="rounded border border-stone-200 border-dashed p-8 text-center text-sm text-stone-400">
          No outflow data available for {entity.display} in {year}.
        </div>
      )}
      ...
      {!loading && groups.length > 0 && (
        <>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-stone-500">{records.length} payments across {groups.length} scheme{groups.length !== 1 ? 's' : ''}</span>
